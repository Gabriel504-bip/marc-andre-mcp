...
describe('redact', () => {
  it('masque les clés sensibles', () => {
    const out = redact({ adminKey: 'abc', staffToken: 'xyz', name: 'Marcel' }) as any;
  ...
  it('masque un token GitHub trouvé dans une chaîne libre', () => {
    const out = redact('erreur: github_pat_11CF2FNEI0DQLduy7bMnH5b2cBhZxPXyLqcQOJEraTlPo0w') as string;
    expect(out).not.toContain('11CF2F');
    expect(out).toContain('[REDACTED]');
  });

  it('descend récursivement dans les objets imbriqués et les tableaux', () => {
    const out = redact({ list: [{ password: 'secret', ok: true }] }) as any;
