...
const parse = (extra: Record<string, unknown> = {}) =>
  ecritureManuelleCreer.inputSchema.parse({ ...base, reference: 'ref-test', ...extra }) as any;
...
describe('ma_ecriture_manuelle_creer — reference obligatoire (idempotence)', () => {
  ...
  it('accepte une reference non vide', () => {
    expect(parse({ reference: 'das-federal-2026-07-entrenous' }).reference).toBe(
      'das-federal-2026-07-entrenous'
    );
  });
});
...
describe('ma_ecriture_manuelle_creer — csvText obligatoire', () => {
  it('refuse un csvText vide ou absent', () => {
    ...
    expect(() =>
      ecritureManuelleCreer.inputSchema.parse({ sessionToken: base.sessionToken, reference: 'r', csvText: '' })
    ).toThrow();
  });
});
