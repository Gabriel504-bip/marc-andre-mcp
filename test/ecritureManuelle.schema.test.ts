...
const parse = (extra: Record<string, unknown> = {}) =>
  ecritureManuelleCreer.inputSchema.parse({ ...base, ecritures: [item()], ...extra }) as any;
...
describe('ma_ecriture_manuelle_creer — simulation par défaut', () => {
  ...
  it("une valeur inattendue retombe du côté SÛR", () => {
    for (const v of [0, 1, 'oui', 'non', null, {}, [], 'nope']) {
      expect(parse({ simulation: v as unknown }).simulation).toBe(true);
    }
  });
});
...
describe('ma_ecriture_manuelle_creer — reference obligatoire par écriture', () => {
  it('refuse une écriture sans reference', () => {
    ...
    expect(() =>
      ecritureManuelleCreer.inputSchema.parse({ ...base, ecritures: [{ csvText: item().csvText }] })
    ).toThrow();
  });

  it('accepte une reference non vide', () => {
    expect(
      parse({ ecritures: [item({ reference: 'das-federal-2026-07-entrenous' })] }).ecritures[0].reference
    ).toBe('das-federal-2026-07-entrenous');
  });
});
...
describe('ma_ecriture_manuelle_creer — csvText obligatoire par écriture', () => {
  it('refuse un csvText vide ou absent', () => {
    ...
    expect(() =>
      ecritureManuelleCreer.inputSchema.parse({ ...base, ecritures: [{ reference: 'r' }] })
    ).toThrow();
  });
});
...
describe('ma_ecriture_manuelle_creer — code de taxe (correctif 2026-08-15)', () => {
  it("le champ csvText documente la colonne optionnelle Code de taxe et corrige l'ancienne fausse affirmation", () => {
    const csvTextDesc = (ecritureManuelleCreer.inputSchema as any).shape.ecritures.element.shape.csvText.description;
...
describe('ma_ecriture_manuelle_publier — confirmation par défaut', () => {
  const parseP = (extra: Record<string, unknown> = {}) =>
    ecritureManuellePublier.inputSchema.parse({ ...base, references: ['ref-test'], ...extra }) as any;
  ...
  it('une valeur inattendue retombe du côté SÛR (jamais publiée par accident)', () => {
    for (const v of [0, 1, 'oui', 'non', null, {}, [], 'nope', false, 'false']) {
      expect(parseP({ confirmation: v as unknown }).confirmation).toBe(false);
    }
  });
});
