...
describe('ma_qbo_inventaire_ajuster — garde-fous d\'approbation', () => {
  ...
  it('traite une confirmation ambiguë comme un refus, jamais comme un accord', () => {
    for (const v of ['oui', 'yes', '1', 'TRUE ', 0, null, undefined, {}]) {
      const r = qboInventaireAjuster.inputSchema.parse({ ...baseAj, confirmation: v }) as any;
      if (String(v).trim().toLowerCase() === 'true') expect(r.confirmation).toBe(true);
      else expect(r.confirmation).toBe(false);
    }
  });

  it('accepte « true » en chaîne (tolérant sur la forme, strict sur le fond)', () => {
    const r = qboInventaireAjuster.inputSchema.parse({ ...baseAj, confirmation: 'true' }) as any;
...
describe('registre', () => {
  it('expose les deux nouveaux outils sous des noms uniques', () => {
    const noms = buildToolList({ allowTier2: true } as never).map((t: any) => t.name);
