...
export const facturationApercu = {
    name: 'ma_facturation_apercu',
    tier: 2,
    description: "Aperçu d'un import de facturation (max 12 fichiers CSV). AUCUNE persistance à cette étape. " +
...
export const facturationRapprocherExecuter = {
    name: 'ma_facturation_rapprocher_executer',
    tier: 2,
    description: "Exécute le rapprochement de facturation préparé par ma_facturation_apercu. Exige le " +
...
export async function runFacturationExecuter(input, ctx) {
    const apercu = consumeIntent('facturation', undefined, input.confirmation_token);
    const startResult = await ctx.http.invoke('facturation_job_start', { invoices: apercu.invoices, importSummary: apercu.summary }, ctx.requestId);
    const final = await driveJobToCompletion(ctx.http, 'facturation_job_step', { jobId: startResult.jobId }, ctx.requestId, ctx.logger);
    return { jobId: startResult.jobId, status: final.status, compteurs: final.counts };
}
