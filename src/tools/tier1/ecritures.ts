...
export const ecritures: ToolDefinition = {
  name: 'ma_ecritures',
  tier: 1,
  description:
    'Bac de révision (factures, états de compte, relevés) tous clients ou un client précis. ' +
  ...
  action: 'ecritures',
  postProcess: (raw: any, input: any) => {
    const lines = Array.isArray(raw?.lines) ? raw.lines : [];
    return {
      ...raw,
      lines: input.inclureRaw ? lines : stripRawFields(lines),
      avertissement:
        raw?.doublonStats?.tablesTronquees?.length > 0
...
export const statutJob: ToolDefinition = {
  ...
  tier: 1,
  description:
    "Statut d'un job d'analyse QBO. ⚠️ La route sous-jacente peut renvoyer HTTP 200 avec un champ " +
  ...
  action: 'statut_job',
  postProcess: (raw: any, input: any) => {
    if (input.inclureResultats) return raw;
