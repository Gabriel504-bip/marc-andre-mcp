...
export const qboAnalysePreparer: ToolDefinition = {
  name: 'ma_qbo_analyse_preparer',
  tier: 2,
  description:
    "Étape 1/2 — prépare une analyse QuickBooks pour une période (aucun effet, réversible). " +
...
export const qboAnalyseExecuter: ToolDefinition = {
  name: 'ma_qbo_analyse_executer',
  tier: 2,
  description:
    'Étape 2/2 — exécute l\'analyse QuickBooks préparée par ma_qbo_analyse_preparer. Exige le ' +
 ...
 */
export async function runQboAnalyseExecuter(
  input: { confirmation_token: string },
  ctx: ToolContext
) {
  const params = consumeIntent('qbo_analyse', undefined, input.confirmation_token) as z.infer<
    typeof prepareInput
  >;
  ...
  const final = await driveJobToCompletion(
    ctx.http,
    'qbo_job_step',
    { sessionToken: params.sessionToken, jobId: startResult.jobId },
    ctx.requestId,
    ctx.logger
  );
  ...
  return {
    ...
    status: final.status,
    compteurs: (final as any).counts,
  };
}

export const qboAnalyseProgression: ToolDefinition = {
  name: 'ma_qbo_analyse_progression',
  tier: 2,
  description:
    "Reprend/consulte la progression d'une analyse QBO en cours (utile si la boucle a été " +
