...
export const relancePreparer: ToolDefinition = {
  name: 'ma_relance_preparer',
  tier: 2,
  description:
    "Prépare une relance client (courriel). AUCUN envoi à cette étape. Affiche toujours le " +
  ...
  postProcess: (raw: any, input) => {
    const dernieresRelances: string[] = raw?.relanceDates ?? [];
    const recente = dernieresRelances.some(
      (d) => Date.now() - new Date(d).getTime() < 24 * 60 * 60 * 1000
    );
    if (recente) {
      throw new ConfirmationError(
        'Une relance a déjà été envoyée à ce client dans les 24 dernières heures ' +
...
export const relanceExecuter: ToolDefinition = {
  name: 'ma_relance_executer',
  tier: 2,
  description:
    'Envoie RÉELLEMENT le courriel de relance préparé par ma_relance_preparer (ou à l\'adresse de ' +
...
export async function runRelanceExecuter(input: { confirmation_token: string }, ctx: ToolContext) {
