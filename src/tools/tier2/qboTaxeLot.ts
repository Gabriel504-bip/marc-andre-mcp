...
const qboTaxeLotAppliquerInput = z.object({
  ...
  confirmation: confirmationTolerante
    .default(false)
    .describe(
      "FAUX (défaut) = aperçu : compte ce qui reste à traiter et montre quelques exemples, sans rien " +
        'modifier. Mettre VRAI traite RÉELLEMENT le lot dans QuickBooks.'
    ),
});

export const qboTaxeLotAppliquer: ToolDefinition = {
  name: 'ma_qbo_taxe_lot_appliquer',
  tier: 2,
  description:
    "Applique un code de taxe EN LOT à toutes les transactions de revenu d'une période qui ne l'ont " +
