...
const transactionReleve = z.object({
  ...
  soldeApres: z
    ...
    .optional()
    .describe(
      'Solde courant imprimé après cette ligne, si le relevé a une colonne de solde. FORTEMENT ' +
...
export const releveDeposer: ToolDefinition = {
  name: 'ma_releve_deposer',
  tier: 2,
  description:
    "Dépose dans la session d'un client un relevé bancaire que tu as LU dans la conversation, pour " +
  ...
  inputSchema: z.object({
    ...
    remplacer: z
      ...
      .optional()
      .describe(
        'FAUX (défaut) : refuse si un relevé est déjà extrait pour cette période, en indiquant ce ' +
