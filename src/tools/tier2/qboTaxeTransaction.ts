...
const qboTaxeTransactionModifierInput = z.object({
  ...
  transactionId: z
    ...
    .min(1)
    .describe(
      "Identifiant QuickBooks de la transaction à modifier (l'Id interne, pas le numéro de " +
  ...
  affichageMontants: z
    ...
    .optional()
    .describe(
      'Menu « Affichage des montants » de QuickBooks : taxe-non-comprise (TaxExcluded), ' +
  ...
  codeTaxe: z
    ...
    .optional()
    .describe(
      "Nom EXACT d'un code de taxe QuickBooks (voir ma_qbo_listes_reference, ex. « TPS/TVQ QC - 9,975 ») " +
        'à appliquer sur TOUTES les lignes de la transaction. Omettre pour ne pas y toucher.'
    ),
  montantBanque: nombreTolerant
    .optional()
    .describe(
      "Montant RÉELLEMENT passé au compte bancaire (celui qui est apparié au fil bancaire). Remis " +
  ...
  confirmation: confirmationTolerante
    .default(false)
    .describe(
      "FAUX (défaut) = aperçu : relit la transaction réelle et montre son état actuel, rien n'est " +
...
export const qboTaxeTransactionModifier: ToolDefinition = {
