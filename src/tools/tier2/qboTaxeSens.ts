...
const qboTaxeLotCorrigerSensInput = z.object({
  ...
  sensTaxe: z
    ...
    .default('ventes')
    .describe(
      'Côté visé : « ventes » (TaxApplicableOn: Sales, taxe PERÇUE — le cas des revenus) ou ' +
  ...
  confirmation: confirmationTolerante
    .default(false)
    .describe(
      "FAUX (défaut) = aperçu : compte celles du mauvais côté et celles déjà correctes, sans rien " +
        'modifier. VRAI bascule RÉELLEMENT le lot.'
    ),
});

export const qboTaxeLotCorrigerSens: ToolDefinition = {
  name: 'ma_qbo_taxe_lot_corriger_sens',
  tier: 2,
  description:
    'Bascule EN LOT le CÔTÉ DU REGISTRE des transactions déjà taxées : fait passer leur taxe de ' +
