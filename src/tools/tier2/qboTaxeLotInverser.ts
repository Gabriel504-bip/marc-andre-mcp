...
const qboTaxeLotInverserInput = z.object({
  ...
  montantsExplicites: z
    ...
    .optional()
    .describe(
      "Montants d'origine imposés, par identifiant de transaction : { \"577\": 2931.20 }. À utiliser " +
  ...
  confirmation: confirmationTolerante
    .default(false)
    .describe(
      "FAUX (défaut) = aperçu : compte ce qui reste à inverser, montre des exemples avec le montant " +
        'qui serait remis dans la ligne, et liste les cas à reprise manuelle. Rien n\'est modifié. ' +
        'VRAI inverse RÉELLEMENT le lot.'
    ),
});

export const qboTaxeLotInverser: ToolDefinition = {
  name: 'ma_qbo_taxe_lot_inverser',
  tier: 2,
  description:
    "Annule EN LOT l'application d'un code de taxe : remet les transactions « hors champ de la taxe » " +
