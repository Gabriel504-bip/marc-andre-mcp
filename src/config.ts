...
function required(name: string): string {
  ...
  if (!v || v.trim() === '') {
    throw new Error(
      `[config] Variable d'environnement manquante ou vide : ${name}. ` +
...
export interface Config {
  ...
  cacheTtlMs: number;
  logFile: string;
  allowedClients: string[] | null; // null = pas de restriction (décision D8)
}
...
export function loadConfig(): Config {
  ...
  if (cfg.mcpBearerToken.length < 32) {
    throw new Error(
      '[config] MCP_BEARER_TOKEN est trop court (< 32 caractères). Génère un secret plus long.'
    );
  }
  ...
  if (!/^https?:\/\//i.test(cfg.maBaseUrl)) {
    throw new Error(
      `[config] MA_BASE_URL doit commencer par http:// ou https:// (valeur reçue : "${cfg.maBaseUrl}"). ` +
